...
Run **`Mocklify: Select AI Provider`** to switch, or leave the `mocklify.ai.provider` setting on `auto` (Copilot → Claude → OpenAI → Gemini, first available wins). Models are configurable via `mocklify.ai.claudeModel` (default `claude-opus-4-8`), `mocklify.ai.openaiModel`, and `mocklify.ai.geminiModel`. Keys are stored in VS Code's encrypted secret storage.
